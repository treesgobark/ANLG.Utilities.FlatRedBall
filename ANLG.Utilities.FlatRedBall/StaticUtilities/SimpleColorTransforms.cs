...
public static class SimpleColorTransforms
{
    ...
    public static double[] RgBtoHsl(Color color)
    {
        ...
        else
        {
            s = l switch
            {
                > 0D and <= .5D => (max - min) / (max + min),
                > .5D => (max - min) / (2D - (max + min)),
                _ => s,
            };
        }
    ...
    public static double[] RgBtoHsb(Color color)
    {
        ...
        double h = 0D;
        if (SystemMath.Abs(max - r) < tolerance
            && g >= b)
            h = 60D * (g - b) / (max - min);
        else if (SystemMath.Abs(max - r) < tolerance
                 && g < b)
        ...
        double s = SystemMath.Abs(max) < tolerance
            ? 0D
            : 1D - min / max;
        ...
        byte? outputAlpha = null)
    {
        double[] hsl = colorTransformMode == ColorTransformMode.Hsl
            ? RgBtoHsl(color)
            : RgBtoHsb(color);
        if (SystemMath.Abs(hsl[2]) < tolerance
        ...
        byte? outputAlpha = null)
    {
        double[] hsl = colorTransformMode == ColorTransformMode.Hsl
            ? RgBtoHsl(color)
            : RgBtoHsb(color);
        if (SystemMath.Abs(hsl[1]) < tolerance
